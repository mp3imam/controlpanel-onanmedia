                ...
                <div class="card-body">
                    <div id="customerList">
                        <div class="col-sm-auto mb-3">
                            ...
                            @else
                                <button id="create_filter" type="button"
                                    class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                    Create ({{ $create }})
                                </button>
                            @endhasrole
                            <button id="on_progress_filter" type="button"
                                class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                On Progress ({{ $on_progress }})
                            </button>
                            <button id="prosess_filter" type="button"
                                class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                Prosess ({{ $prosess }})
                            </button>
                            <button id="pending_filter" type="button"
                                class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                Pending ({{ $pending }})
                            </button>
                            <button id="cancell_filter" type="button"
                                class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                Cancel ({{ $tolak }})
                            </button>
                            <button id="done_filter" type="button"
                                class="btn px-4 mx-1 bg-animation waves-effect waves-light rounded-5">
                                Done ({{ $histori }})
                            </button>
                                ...
                                <div class="row">
                                    <div class="col-md-4 p-3">
                                        ...
                                        <input type="text" class="form-control flatpickr-input" id="tanggal"
                                            ...
                                            value="{{ Carbon\Carbon::now()->subMonths(3)->firstOfMonth()->format('d M, Y') . ' to ' . Carbon\Carbon::now()->format('d M, Y') }}">
                                    </div>
                                    ...
                                    <div class="col-md py-2 text-center mt-4 float-end">
                                        <a href="{{ route('master_kas_belanja.create') }}" type="button"
                                            class="btn text-white fs-20 px-4" style="background-color: #4E36E2">
                                            ...
                                            <tfoot id="footer_nominal">
                                                <tr>
                                                    <td class="text-end" colspan="8">
                                                        ...
                                                        <label class="fs-20 text-white rounded-4 py-2 px-5 pt-2"
                                                            style="background-color: #4E36E2"
                                                            id="checked_sum">{{ $checked_sum }}</label>
                                                    </td>
                                                    <td class="text-end mb-2" colspan="2">
                                                        <button id="approved"
                                                            class="btn btn-success fs-20 text-white rounded-4 py-2 px-4"
                                                            onclick="konfirmasi_id('{{ $checked_id }}','{{ $checked_sum }}')">
                                                            Approved
                                                        </button>
                                                    </td>
                                                </tr>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $(function() {
            var table = $('#dataTable').DataTable({
                ...
                columns: [{
                    ...
                    name: 'No. Transaksi',
                    render: function(data, type, row, meta) {
                        return '<a href="' + route + "/" + row.id + '/edit?q=' + $('#q').val() +
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        var nominalLabel = `<label class="nominal_approve">` + data +
                            `</label>`;
                        // Memastikan elemen '.nominal_approve' ada di DOM sebelum memformatnya
                        $(document).ready(function() {
                            $('.nominal_approve').priceFormat({
                                prefix: 'Rp. ',
                                centsSeparator: ',',
                                thousandsSeparator: '.',
                                centsLimit: 0
                    ...
                    name: 'Action',
                    render: function(data, type, row, meta) {
                        button = `<a type="button" href="{{ url('master_kas_belanja') }}/` +
                            row.id +
                            `/edit" class="btn btn-warning btn-icon waves-effect waves-light"><i class="ri-pencil-fill" target="_blank"></i></a>
        ...
        // Fungsi untuk memformat angka menjadi mata uang (misal: 1000 -> Rp1.000)
        ...
        function konfirmasi_id(id, nominal) {
            Swal.fire({
                ...
                text: "Transaksi Kas sebesar " + formatCurrency(nominal),
                icon: "warning",
                showCancelButton: true,
