    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="col-md-11">
                    <h4 class="card-title mb-0">Detail <span
                            style="color:#4E36E2">{{ $detail->nomor_transaksi . ' => ' . $detail->dokumen }}</span></h4>
                </div>
                <div class="col">
                    <div class="row float-end">
                        <div class="col">
                            @if ($detail->jenis == 0)
                                <button type="button"
                                    class="btn btn-outline-danger btn-icon waves-effect waves-light float-end"
                                    onclick="konfirmasi_hapus('{{ $detail->id }}',`{{ $detail->nomor_transaksi }}`)"
                                    target="_blank"><i class="ri-delete-bin-5-line"></i></button>
                            @endif
                        </div>
                        <div class="col">
                            <a href="{{ route('master_jurnal.index') }}"
                                class="btn bg-animation rounded-5 btn-outline-primary waves-effect waves-light float-end"
                                style="color: #4E36E2"><i class="ri-arrow-go-back-line"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('master_jurnal.update', $detail->id) }}" method="POST"
                    ...
                    <div class="row">
                        <div class="row mb-3">
                                    ...
                                    <div class="carousel-indicators">
                                        @foreach ($detail->jurnal_file as $foto => $f)
                                            <button id="carousel-role{{ $f->id }}" type="button"
                                                ...
                                                data-bs-slide-to="{{ $foto }}" class="active"
                                                aria-current="true" aria-label="Slide {{ $foto }}"></button>
                                        @endforeach
                                    </div>
                                    <div class="carousel-inner">
                                        @foreach ($detail->jurnal_file as $foto => $f)
                                            <div id="carousel{{ $f->id }}"
                                                class="carousel-item {{ $foto == 0 ? 'active' : '' }}"
                                                data-bs-interval="2000">
                                                <img id="{{ $f->id }}" class="d-block" width="100%"
                                                    height="300px" src="{{ asset($f->path) . '/' . $f->filename }}">
                                                <div class="carousel-caption d-none d-md-block">
                                                    <a href="{{ asset($f->path) . '/' . $f->filename }}"
                                                        target="_blank"
                            ...
                            @if ($detail->jenis == 0)
                                <div class="col-md-12">
                                    <input class="bg-success" type="file" name="attachment[]" id="attachment"
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="date" class="form-control" id="tanggal_transaksi" name="tanggal_transaksi"
                                value="{{ $detail->tanggal_transaksi }}" {{ $detail->jenis == 0 ? '' : 'disabled' }}
                                required />
                        </div>
                        {{--
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <textarea class="form-control" id="keterangan_jurnal_umum" name="keterangan_jurnal_umum" rows="1"
                                {{ $detail->jenis == 0 ? '' : 'disabled' }}>{{ $detail->keterangan_jurnal_umum }}</textarea>
                        </div>
                    </div>
                                        ...
                                        @if ($detail->jenis == 0)
                                            <button class="btn float-end" type="button" onclick="tambah_detail()"
                                                style="background-color:#E0E7FF; color:#4E36E2"><i
                                                    class="ri-add-box-fill"></i> Tambah Baris</button>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="card-body tambah_detail">
                                    @foreach ($detail->details as $belanja => $b)
                                        <div class="row delete_detail mt-2">
                                            <div class="col-md-3">
                                                <select id="akun_belanja{{ $b->id }}" name="akun_belanja[]"
                                                    class="form-control akun_belanja"
                                                    {{ $detail->jenis == 0 ? '' : 'disabled' }} required></select>
                                            </div>
                                            <div class="col">
                                                <input id="keterangan[]" name="keterangan[]" class="form-control"
                                                    value="{{ $b->keterangan }}"
                                                    {{ $detail->jenis == 0 ? '' : 'disabled' }} />
                                            </div>
                                            <div class="col">
                                                <input class="form-control text-end debet" id="debet[]"
                                                    name="debet_detail[]" value="{{ $b->debet ?? '0' }}"
                                                    onkeyup="countDebet()" {{ $detail->jenis == 0 ? '' : 'disabled' }}
                                                    required />
                                            </div>
                                            <div class="col">
                                                <input class="form-control text-end kredit" id="kredit[]"
                                                    name="kredit_detail[]" value="{{ $b->kredit ?? '0' }}"
                                                    onkeyup="countKredit()"
                                                    {{ $detail->jenis == 0 ? '' : 'disabled' }} required />
                                            </div>
                                            @if ($detail->jenis == 0 && $belanja !== 0)
                                                <div class="col">
                                ...
                                <div class="card-footer">
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            <input class="form-control text-end total text-white"
                                                style="background-color: #4E36E2" id="total_debet" name="total_debet"
                                                value="{{ $detail->debet }}" readonly />
                                        </div>
                                        <div class="col">
                                            <input class="form-control text-end total text-white"
                                                style="background-color: #4E36E2" id="total_kredit"
                                                name="total_kredit" value="{{ $detail->kredit }}" readonly />
                                        </div>
                                        @if ($detail->jenis == 0 && $belanja !== 0)
                                            <div class="col">
                                                <div id="ubahwarna" class="col text-white text-end rounded-3"
                                                    style="background-color: #4E36E2; padding-right: 20px">
                                                    <label class="mt-2" id="total_all" name="total_all">Rp.
                                                        0</label>
                                                </div>
                                            </div>
                                        @endif
                    ...
                    <div class="float-end">
                        @if ($detail->jenis == 0)
                            <button class="btn btn-success mr-5 rounded-5 bg-animation bg-success"><i
                                    class="ri-ball-pen-fill label-icon align-middle fs-16 me-2"></i> Ubah</button>
                            &nbsp;&nbsp;&nbsp;
                        @endif
                        <a href="{{ route('master_jurnal.index') }}"
                            class="btn bg-animation rounded-5 btn-outline-primary waves-effect waves-light float-end"
...
<script>
    ...
    $.each(f, function(i, item) {
        ...
        if (i !== 1 && s == 3 || i == f.length - 1 && s == 4 || i == f.length - 1 && s == 2 || i == f.length -
            1 && s == 5) {
            dataId = item.jurnal_banks.id
            dataText = item.jurnal_banks.nama
        } else if (i !== 1 && s == 6) {
            dataId = item.jurnal_banks_public.id
            dataText = item.jurnal_banks_public.nama
        }
    ...
    function convertRupiah() {
        ...
        $('#total_all').text(parseInt($('#total_debet').val().replace("Rp. ", "").replaceAll(",", "").replaceAll(".",
            "")) - parseInt($('#total_kredit').val().replace("Rp. ", "").replaceAll(",", "").replaceAll(".",
            "")))
        $('#total_all').priceFormat({
            prefix: 'Rp. ',
            centsSeparator: ',',
            thousandsSeparator: '.',
            centsLimit: 0,
        ...
        $('#total_all').text() !== "Rp. 0" ? $('#ubahwarna').attr('style',
            'background-color : #E9967A; padding-right: 20px') : $('#ubahwarna').attr('style',
            'background-color : #4E36E2; padding-right: 20px')
    }
