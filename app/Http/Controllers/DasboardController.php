...
class DasboardController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $jumlah_satker = (int) 1; //SatkerModel::get()->count();

        // Menghitung Jumlah Satker yang aktif
        $kiosk_aktif  = (int) 1; //LogActivitiesModel::where('created_at', '>', Carbon::now()->format('Y-m-d').' 00:00:00')->groupBy('url')->groupBy(DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d')"))->whereSubject('Membuka KiosK')->get()->count();

        // Menghitung Admin yang membuka Layanan
        $satker_aktif = (int) 1; //LogActivitiesModel::where('created_at', '>', Carbon::now()->format('Y-m-d').' 00:00:00')->groupBy('user_id')->groupBy(DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d')"))->where('Subject','Buka Booking Layanan')->where('user_id','!=',3)->orWhereNull('user_id')->get()->count();
